...
### Local Modifications (Tabs UI & Responsive) - 2025-09-14

Changes specific to this fork; applies to `interface/` and `templates/` only.

- `interface/main/tabs/js/tabs_view_model.js`
  - Keep active tab in visible row during overflow recalculation; ensure at least one visible tab.
  - Swap rule in `activateTab()` so selecting from “More” brings tab into the row immediately.

- `templates/interface/main/tabs/tabs_template.html.twig`
  - Removed `with: tabs`; bind directly via `tabs.visibleTabs`/`tabs.moreMenuVisible`.
  - Always render tab titles; fixed “More” items to prevent default and trigger `tabClicked`.

- `interface/main/tabs/main.php`
  - CSS to avoid dropdown clipping; raised stacking; portal “More” menu to `<body>` on open, restore on close.
  - Width normalization so tab bar can overflow; cache-buster for `custom_bindings.js`.

- `interface/main/tabs/js/custom_bindings.js`
  - Inject viewport meta + responsive CSS at `max-width: 992px` into same-origin tab content.
  - Equalize 2–4 inline inputs via `.oemr-eq` grid; re-run on delay and resize.
  - Propagate injection to same-origin nested frames/iframes inside tab content.
