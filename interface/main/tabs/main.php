...
<head>
    ...
    <script src="js/custom_bindings.js?v=<?php echo $v_js_includes; ?>&cb=<?php echo @filemtime(__DIR__ . '/js/custom_bindings.js'); ?>"></script>
    ...
    <style>
        ...
        /* Ensure the tab header uses the available width of the viewport */
        #tabs_div,
        #tabs_div .tabControls {
            width: 100%;
        }
        /* Let dropdown overflow outside the header container */
        #tabs_div {
            overflow: visible !important;
            position: relative;
            /* Raise above side menu dropdowns which may use very high z-indices */
            z-index: 0;
        }
        /* Ensure the tabs header does not clip the More dropdown */
        #tabs_div .tabControls {
            overflow: visible !important;
        }
        /* Keep the More dropdown above adjacent content */
        #tabs_div .tabs-more {
            position: relative;
            z-index: 0;
        }
        #tabs_div .tabs-more .dropdown-menu {
            z-index: 0;
        }
    </style>
</head>
...
?>
    ...
    <script>
        ...
        (function() {
            try {
                if (app_view_model && app_view_model.application_data && app_view_model.application_data.tabs && typeof app_view_model.application_data.tabs.scheduleRecalc === 'function') {
                    // initial pass after bindings
                    app_view_model.application_data.tabs.scheduleRecalc();
                    // another pass after a tick to allow DOM to settle
                    setTimeout(function(){ app_view_model.application_data.tabs.scheduleRecalc(); }, 0);
                    // recalc on viewport changes
                    window.addEventListener('resize', function(){ app_view_model.application_data.tabs.scheduleRecalc(); });
                }
            } catch (e) { /* no-op */ }
        })();
        $(function() {
        ...
        // Ensure the Tabs "More" dropdown overlays any side menus by portaling to <body>
        (function() {
            var $doc = $(document);
            $doc.on('show.bs.dropdown', '.tabs-more', function() {
                var $container = $(this);
                var $toggle = $container.find('.tabs-more-toggle');
                var $menu = $container.find('.dropdown-menu');
                var off = $toggle.offset();
                var top = off.top - $(window).scrollTop() + $toggle.outerHeight();
                var left = off.left - $(window).scrollLeft();
                // Move to body with fixed positioning to escape stacking contexts
                $menu.appendTo('body')
                    .css({position:'fixed', top: top + 'px', left: left + 'px', display:'block', zIndex: 5});
            });
            $doc.on('hide.bs.dropdown', '.tabs-more', function() {
                var $container = $(this);
                var $menu = $('body > .dropdown-menu[aria-labelledby="tabsMoreDropdown"]');
                // Restore to original container and clear positioning
                if ($menu.length) {
                    $menu.removeAttr('style').appendTo($container);
                }
            });
        })();
