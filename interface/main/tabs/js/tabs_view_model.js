...
function tabStatus(title,url,name,loading_label,closable,visible,locked)
{
    ...
    self.title.subscribe(function() {
        ...
        //Stop Spinning motor
        self.spinner("");
        // Title change may alter tab width; schedule layout recalculation if available
        try {
            if (window.app_view_model && app_view_model.application_data && app_view_model.application_data.tabs && typeof app_view_model.application_data.tabs.scheduleRecalc === 'function') {
                app_view_model.application_data.tabs.scheduleRecalc();
            }
        } catch (e) {}
    });
    self.window=null;
...
{
    var self = this;
    self.tabsList = ko.observableArray();
    // Arrays for responsive overflow handling (render-only lists)
    self.visibleTabs = ko.observableArray();
    self.overflowTabs = ko.observableArray();
    self.moreMenuVisible = ko.computed(function() { return self.overflowTabs().length > 0; });

    // Debounced layout recalculation
    self._recalcTimer = null;
    self.scheduleRecalc = function() {
        if (self._recalcTimer) {
            clearTimeout(self._recalcTimer);
        }
        self._recalcTimer = setTimeout(function() {
            self._recalcTimer = null;
            self.recalculateTabsLayout();
        }, 50);
    };

    self.recalculateTabsLayout = function() {
        try {
            var container = document.querySelector('#tabs_div .tabControls');
            if (!container) { return; }

            // Start with all tabs visible and none in overflow
            var allTabs = self.tabsList();
            self.visibleTabs(allTabs.slice(0));
            self.overflowTabs([]);

            var fitOnce = function() {
                var overflows = container.scrollWidth > (container.clientWidth + 1);
                if (overflows) {
                    var visible = self.visibleTabs();
                    if (!visible || visible.length === 0) { return; }
                    // Prefer to move the last non-active tab
                    var activeIndex = -1;
                    for (var i = 0; i < visible.length; i++) {
                        if (typeof visible[i].visible === 'function' && visible[i].visible()) { activeIndex = i; break; }
                    }
                    var idxToMove = visible.length - 1;
                    if (idxToMove === activeIndex && visible.length > 1) { idxToMove = visible.length - 2; }
                    var moved = visible.splice(idxToMove, 1)[0];
                    self.visibleTabs(visible.slice(0));
                    var ov = self.overflowTabs();
                    ov.unshift(moved);
                    self.overflowTabs(ov);
                    ...
                    // Ensure the active tab (where model.visible() === true) is in the visible list
                    var active = null;
                    for (var j = 0; j < allTabs.length; j++) {
                        if (typeof allTabs[j].visible === 'function' && allTabs[j].visible()) { active = allTabs[j]; break; }
                    }
                    if (active) {
                        var vlist = self.visibleTabs();
                        var olist = self.overflowTabs();
                        var vHas = vlist.indexOf(active) !== -1;
                        if (!vHas) {
                            // Make room by moving the last non-active visible to overflow if needed
                            if (vlist.length > 0) {
                                var moveIdx = vlist.length - 1;
                                if (typeof vlist[moveIdx].visible === 'function' && vlist[moveIdx].visible() && vlist.length > 1) {
                                    moveIdx = vlist.length - 2;
                                }
                                var moved2 = vlist.splice(moveIdx, 1)[0];
                                olist.unshift(moved2);
                            }
                            vlist.push(active);
                            // Remove from overflow if present
                            var oi = olist.indexOf(active);
                            if (oi !== -1) { olist.splice(oi, 1); }
                            self.visibleTabs(vlist.slice(0));
                            self.overflowTabs(olist);
                        }
                    }
                    // Safety: ensure at least one visible tab remains
                    if (self.visibleTabs().length === 0 && allTabs.length > 0) {
                        var vf = allTabs[0];
                        var ov2 = self.overflowTabs();
                        var oi2 = ov2.indexOf(vf);
                        if (oi2 !== -1) { ov2.splice(oi2, 1); }
                        self.visibleTabs([vf]);
                        self.overflowTabs(ov2);
                    }
                }
            };
    ...
    // Recalculate any time the underlying tab list changes
    self.tabsList.subscribe(function() { self.scheduleRecalc(); });
...
function activateTab(data)
{
    ...
    // Swap rule: if the activated tab is in overflow list, swap it into the visible row
    try {
        var tabsModel = app_view_model.application_data.tabs;
        if (tabsModel && typeof tabsModel.visibleTabs === 'function' && typeof tabsModel.overflowTabs === 'function') {
            var v = tabsModel.visibleTabs();
            var o = tabsModel.overflowTabs();
            var vin = v.indexOf(data) !== -1;
            var oin = o.indexOf(data) !== -1;
            if (!vin && oin) {
                // Move a non-active visible tab to overflow to make room
                if (v.length > 0) {
                    var moveIdx = v.length - 1;
                    if (typeof v[moveIdx].visible === 'function' && v[moveIdx].visible() && v.length > 1) {
                        moveIdx = v.length - 2; // don't evict the current active if it's last
                    }
                    var moved = v.splice(moveIdx, 1)[0];
                    o.unshift(moved);
                }
                // Remove activated from overflow and append to visible
                var oi = o.indexOf(data);
                if (oi !== -1) { o.splice(oi, 1); }
                v.push(data);
                tabsModel.visibleTabs(v.slice(0));
                tabsModel.overflowTabs(o);
            }
        }
    } catch (e) { /* no-op */ }
    try { app_view_model.application_data.tabs.scheduleRecalc(); } catch(e) {}
}
